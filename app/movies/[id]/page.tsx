...
import { getMovieDetail, getMovieTrailer } from "@/app/api/tmdb";
import { getScoreSingleMovie } from "@/lib/scores";
import {
  getImdbScoreBgColor,
  getMetaScoreBgColor,
  getRtScoreBgColor,
...
export default async function MovieSinglePage({
  ...
  // 作品情報、トレイラー情報の取得
  const [movie, trailerData, score] = await Promise.all([
    getMovieDetail(id),
    getMovieTrailer(id),
    getScoreSingleMovie(Number(id), "movie"),
  ]);
  // トレイラーサイトがYoutubeの最新動画を取得
  const movieTrailer = trailerData.results.filter(
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="hero_title text-neutral-100 font-black text-4xl mt-10 mb-3">
        <h1 className="border-l-6 border-amber-300 pl-5">{movie.title}</h1>
      </div>
      <div className="text-neutral-100 font-semibold text-sm flex gap-5 mb-5">
        <span>上映時間：{movie.runtime}分</span>
      ...
      <div className="flex">
            ...
            <Image
              src={
                movie.backdrop_path
                  ...
                  : `${process.env.NEXT_PUBLIC_TMDB_IMAGE_BASE_URL}/original/${movie.poster_path}`
              }
              width={1920}
              height={1000}
              alt={movie.title}
        ...
        <div className="w-1/4 flex flex-col gap-2 items-center justify-center ml-2">
          <div
            className={`${getTmdbScoreBgColor(movie.vote_average)} text-2xl text-neutral-100 font-semibold flex flex-col justify-center items-center w-full flex-1`}
          >
            <p>TMFDB</p>
